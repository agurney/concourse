                                        ...
                                        case selectedIdx of
                                            ...
                                            Just selectedIdx ->
                                                let
                                                    ...
                                                    selectedItem =
                                                        case ( Array.get selectedIdx options, model.route ) of
                                                            ( Just el, _ ) ->
                                                                el

                                                            ( Nothing, Routes.Dashboard (Routes.Normal (Just q)) ) ->
                                                                q
...
middleSection { route, dropdown, screenSize, groups } =
    case route of
        Routes.Dashboard (Routes.Normal q) ->
            if groups |> List.concatMap .pipelines |> List.isEmpty then
                Empty

            else if dropdown == Hidden && screenSize == Mobile && q == Just "" then
                MinifiedSearch
...
dropdownOptions { route, groups } =
    case route of
        Routes.Dashboard (Routes.Normal query) ->
            case query |> Maybe.map String.trim |> Maybe.withDefault "" of
                "" ->
                    [ "status: ", "team: " ]
